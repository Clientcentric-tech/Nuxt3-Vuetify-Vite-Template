<template>
  <div class="relative" ref="containerRef">
    <div @click="toggleDropdown" class="flex items-center border border-gray-300 rounded px-3 py-1.5" :class="{ 'border-blue-400': opened || focused }">
      ...
      <svg class="transition-all" :class="{ 'rotate-180': opened || focused }" width="15px" height="15px" viewBox="0 0 24 24" fill="none">
    ...
    <div v-if="opened || focused" class="shadow rounded bg-white absolute top-full w-full mt-1 py-1 overflow-hidden">
      <div v-for="option of filteredOptions" :key="option.value" @click="handleSelect(option.value)" class="px-4 py-2 hover:bg-gray-200 cursor-pointer">
...
const filteredOptions = computed(
  () => props.options.filter((option) => (option.label.toLowerCase().includes(search.value.toLowerCase())))
);
